...
logging.basicConfig(filename='/opt/logs/mqtt_moogo_thingsboard_prd.log', level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
#logging.basicConfig(filename='../logs/mqtt_moogo_thingsboard_prd.log', level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
#file_proms = "../key/moogo_thingsboard_mqtt.prom"
file_proms = "/opt/proms/node_exporter/key/moogo_thingsboard_mqtt.prom"

# 配置 ThingsBoard 访问信息
TB_HOST = "tb.moogo.com"
#TB_PORT = 8883
TB_PORT = 1883
TB_ACCESS_TOKEN = "UxQZlxYFRgcSfnEBCXsc"  # 正确的token
...
def on_rpc_response(client, userdata, msg):
    if msg.topic.startswith("v1/devices/me/rpc/response/"):
        ...
        file_data = "mqtt_send_res_thingsboard{app_vego=\"thingsboard-moogo-prd\",env=\"prd\"} 1\n"
        write_to_file(file_proms,file_data)
    else:
        file_data = "mqtt_send_res_thingsboard{app_vego=\"thingsboard-moogo-prd\",env=\"prd\"} 0\n"
        write_to_file(file_proms,file_data)
