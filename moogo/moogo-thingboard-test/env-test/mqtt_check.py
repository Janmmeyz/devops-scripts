...
logging.basicConfig(filename='/opt/logs/mqtt_moogo_thingsboard_test.log', level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
#logging.basicConfig(filename='../logs/mqtt_moogo_thingsboard_test.log', level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
#file_proms = "../key/moogo_thingsboard_mqtt.prom"
file_proms = "/opt/proms/node_exporter/key/moogo_thingsboard_mqtt.prom"

# 配置 ThingsBoard 访问信息
TB_HOST = "tb-test.moogo.com"
TB_PORT = 1883
TB_ACCESS_TOKEN = "8VP85Uoed7p24MTkhlVQ"  # 正确的token
...
def on_rpc_response(client, userdata, msg):
    if msg.topic.startswith("v1/devices/me/rpc/response/"):
        print(f"Received RPC response: {msg.payload.decode()}")
        response = json.loads(msg.payload.decode())
        # 在这里处理 RPC 响应
        # print(response)
        logging.info(response)
        file_data = "mqtt_send_res_thingsboard{app_vego=\"thingsboard-moogo-test\",env=\"test\"} 1\n"
        write_to_file(file_proms,file_data)
    else:
        file_data = "mqtt_send_res_thingsboard{app_vego=\"thingsboard-moogo-test\",env=\"test\"} 0\n"
        write_to_file(file_proms,file_data)
